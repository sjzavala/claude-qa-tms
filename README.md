...
The traceability header is not just documentation — it is a machine-readable coverage map,
and CI can read it. **[playwright-test-selector](https://github.com/sjzavala/playwright-test-selector)**
is a GitHub Action that maps a PR's diff to the specs declaring coverage of the changed
files and runs only those:
...
- run: npx playwright test ${{ steps.select.outputs.specs }}
```

It reads the `@covers` globs and the source paths cited in `@guards` — both of which
`qa-tms:codegen` already emits — so specs generated by this plugin work with it as-is. It
escalates to the full suite whenever narrowing cannot be justified, so it is safe to adopt
incrementally.

The action lives in its own repo and does not require this plugin; the plugin's only job
is to write headers worth reading.
...
├── scripts/validate-plugin.mjs    # structural checks on the plugin itself
